...
pub trait Arg: Sized {
    ...
    fn parse_full(input: &str) -> Result<Self> {
        Self::parse_full_no_pfx(input)
            .map_err(|e| UserError::new(format!("parsing {}: {}", Self::describe_expected(), e)).into())
    }

    fn parse_full_no_pfx(input: &str) -> Result<Self> {
        let (val, rest) = Self::parse_from(input)?;
...
macro_rules! tuple_impls {
    ( $head:ident, $($tail:ident,)* ) => {
        impl<$head: Arg, $( $tail:Arg ),*> Arg for ($head, $($tail),*) {
            fn parse_from(input: &str) -> Result<(Self, Option<&str>)> {
...
impl Arg for () {
    fn parse_from(_: &str) -> Result<(Self, Option<&str>)> {
